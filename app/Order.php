...
class Order extends Model
{
    ...
    protected $fillable = [
        ...
        'update_via_sms',
        'billed',
        'guarantee', 'sub_category', 'subject', 'api_user_id', 'type'
    ];
    ...
    function assignee()
    {
        ...
        // return $this->belongsTo('App\User', 'staff_id', 'api_user_id');
        return $this->belongsTo('App\User', 'api_user_id', 'api_user_id');
    }
    ...
    static function dropdown(string $type = 'order')
    {
        ...
        // $data['service_id_list'] = Service::with(['subCategories'])->orderBy('name', 'ASC')->whereNull('inactive')->get();
        ...
        $urgencies = Urgency::whereNull('inactive')
            ->orderBy('type', 'DESC')->orderBy('value', 'ASC')->orderBy('percentage_to_add', 'ASC')->get();
        $urgency_list = [];
