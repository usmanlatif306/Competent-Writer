...
class SettingsController extends Controller
{
    ...
    {
        $messages = Message::with(['user:id,first_name,last_name', 'receiver:id,first_name,last_name'])->orderBy('created_at', 'DESC');
        ...
        return Datatables::eloquent($messages)
            ...
            ->editColumn('message', function ($message) {
                $url = url("/") . "/storage/" . $message->message;
                return $message->type === 'text' ? $message->message : ("<a href='$url' download>$message->file_name</a>");
            })
            // ("<a href='$message->file_name' download>$message->file_name</a>")
            ->rawColumns([
                'user',
                'writer',
                'message'
